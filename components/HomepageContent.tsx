...
export default function HomepageContent({ articles }: { articles: ArticleMeta[] }) {
    ...
    <>
        ...
        <div className="wrap" style={wrap}>
          ...
          <div className="grid-currently">
            {[
              ...
              { tag: 'Building toward', title: 'Writing consistently',               body: 'Turning patterns I see in performance data into ideas worth publishing. One sharp piece per month.' },
            ].map((c, i) => (
      ...
      <section id="writing" className="section-pad" style={S.section}>
        <div className="wrap" style={wrap}>
          ...
          {articles.map((w, i) => (
            <Link key={w.slug} href={`/writing/${w.slug}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, padding: '28px 0', borderBottom: '1px solid var(--border)', borderTop: i === 0 ? '1px solid var(--border)' : 'none', textDecoration: 'none', color: 'inherit', transition: 'padding-left 0.3s' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-4)', whiteSpace: 'nowrap' }}>
                  {new Date(w.date).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
                </span>
